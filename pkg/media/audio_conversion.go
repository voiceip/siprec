...
// ConcealG729Loss feeds the decoder with frame-erasure flags for lost packets.
// This keeps the decoder's internal state (adaptive codebook, LP synthesis filter)
// in sync so that the next real frame decodes cleanly instead of producing
// distortion from a state mismatch.  Returns the concealment PCM (faded
// replica of the last decoded frame, generated by bcg729).
func ConcealG729Loss(ssrc uint32, lostFrames int) []byte {
	if lostFrames <= 0 {
		return nil
	}
	decoder := g729Pool.GetDecoder(ssrc)
	decoded := make([]int16, 80)
	emptyFrame := make([]byte, 10)

	pcmData := make([]byte, lostFrames*160)
	for i := 0; i < lostFrames; i++ {
		_ = decoder.DecodeWithOptions(emptyFrame, true, false, false, decoded)
		off := i * 160
		for j, s := range decoded {
			binary.LittleEndian.PutUint16(pcmData[off+j*2:], uint16(s))
		}
	}
	return pcmData
}
...
func decodeG729Packet(payload []byte) ([]byte, error) {
	...
	// Handle SID frames (2 bytes for G.729B comfort noise)
	if len(payload) == 2 {
		// G.729B SID frame - use decoder with SID flag
		decoder := g729.NewDecoder()
		defer decoder.Close()
		decoded := make([]int16, 80)
		err := decoder.DecodeWithOptions(payload, false, true, false, decoded)
	...
	// Create decoder (reuse for all frames in packet)
	decoder := g729.NewDecoder()
	defer decoder.Close()
	...
	for frame := 0; frame < numFrames; frame++ {
		...
		err := decoder.Decode(frameData, decoded)
		if err != nil {
			// On error, generate silence for this frame
			for i := 0; i < 80; i++ {
				decoded[i] = 0
			}
		}
